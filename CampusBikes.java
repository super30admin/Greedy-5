//TC : M*N -- m = no. df workers and n = n .of bikes
//SC : O(n) // Using map
...
class Solution {
    public int[] assignBikes(int[][] workers, int[][] bikes) {
        ...
        HashMap<Integer, List<int[]>> map = new HashMap<>(); // Map to store distance to list of indices
        ...
        for(int i = minDist; i <= maxDist; i++){
            ...
            if(li != null){
                for(int[] pair : li){
                    int wk = pair[0];
                    int bk = pair[1];
                    //to assign a bike to worker check first that IFFFFFF   worker haven't assigned to bike yet and bike is not allocated to any worker yet
